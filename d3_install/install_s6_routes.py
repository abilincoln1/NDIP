"""
Install D5A-S6 routes into main.py
Run: docker exec ndip-backend-1 python3 /tmp/install_s6_routes.py
"""
import os, subprocess

MAIN = '/app/app/main.py'

with open(MAIN, 'r') as f:
    content = f.read()

if 'donations_router' in content:
    print('S6 routes already registered in main.py')
else:
    append_block = """
# ── D5A-S6 Donations & Communications Engine ────────────────────────────────
from app.api.routes.donations_comms_v3 import donations_router, comms_router

app.include_router(donations_router)
app.include_router(comms_router)
"""
    with open(MAIN, 'a') as f:
        f.write(append_block)
    print('S6 routes registered in main.py')

# Verify + syntax check
for path in ['/app/app/api/routes/donations_comms_v3.py']:
    exists = os.path.exists(path)
    print(f'{"OK" if exists else "MISSING"}: {path}')
    if exists:
        r = subprocess.run(['python3', '-m', 'py_compile', path], capture_output=True, text=True)
        print(f'Syntax: {"PASS" if r.returncode == 0 else r.stderr}')

print('Done. Watchfiles will auto-reload.')
