@echo off
echo ============================================================
echo  NDIP Phase D2.5 - Seed Data and Test Accounts (Fixed)
echo ============================================================
echo.

set SCRIPT_DIR=%~dp0

echo [1/3] Seeding geography using existing migration file in container...
docker exec ndip-db-1 psql -U agora_user -d agora_db -f /app/migrations/phase_d_00_geography.sql
if %errorlevel% neq 0 (
    echo WARNING: Some notices above are expected - checking if data was seeded...
)

docker exec ndip-db-1 psql -U agora_user -d agora_db -c "SELECT COUNT(*) as states FROM ng_states; SELECT COUNT(*) as lgas FROM ng_lgas;"
...
if %errorlevel% neq 0 (
    echo ERROR: D2.5 seed failed. Check output above.
    pause & exit /b 1
)
echo    Seed complete.
echo.
...
echo.
docker exec ndip-db-1 psql -U agora_user -d agora_db -c "SELECT membership_number, email, role, is_active, is_verified FROM members ORDER BY membership_number;"
