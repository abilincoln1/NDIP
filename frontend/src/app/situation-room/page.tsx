    ...
    <div className="max-w-5xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          {/* V6.0/V6.1 Phase H/I — Strategic Opportunity & Stakeholder
              sections. The backend has returned these fields since V6.0,
              but they were not previously rendered on this page. */}
          {data.strategic_opportunity_snapshot?.top_opportunities?.length > 0 && (
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
              <div className="flex items-center gap-2 mb-4">
                <Target size={16} className="text-teal-400"/>
                <h2 className="text-sm font-bold text-white">Strategic Opportunity Snapshot</h2>
              </div>
              <div className="space-y-3">
                {data.strategic_opportunity_snapshot.top_opportunities.map((o: any, i: number) => (
                  <div key={i} className="p-3 bg-slate-800/40 rounded-lg">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-sm text-white font-medium">{o.title}</span>
                      <span className="text-xs text-white">{o.status}</span>
                    </div>
                    <p className="text-xs text-white">{o.why_it_matters}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {(data.key_stakeholder_watchlist || []).length > 0 && (
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
              <div className="flex items-center gap-2 mb-4">
                <Eye size={16} className="text-purple-400"/>
                <h2 className="text-sm font-bold text-white">Key Stakeholder Watchlist</h2>
              </div>
              <div className="space-y-2">
                {data.key_stakeholder_watchlist.slice(0, 6).map((s: any, i: number) => (
                  <div key={i} className="flex items-center justify-between p-2.5 bg-slate-800/40 rounded-lg">
                    <span className="text-sm text-white">{s.name}</span>
                    <span className="text-xs text-white">{s.mention_count} mentions · {s.monitoring_priority}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {(data.opportunity_momentum || []).length > 0 && (
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
                <div className="flex items-center gap-2 mb-4">
                  <BarChart2 size={16} className="text-blue-400"/>
                  <h2 className="text-sm font-bold text-white">Opportunity Momentum</h2>
                </div>
                <div className="space-y-2">
                  {data.opportunity_momentum.slice(0, 5).map((o: any, i: number) => (
                    <div key={i} className="flex items-center justify-between text-xs">
                      <span className="text-white">{o.name}</span>
                      <span className="text-teal-400 font-bold">{o.mention_count}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {(data.stakeholder_influence_shifts || []).length > 0 && (
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
                <div className="flex items-center gap-2 mb-4">
                  <TrendingUp size={16} className="text-teal-400"/>
                  <h2 className="text-sm font-bold text-white">Stakeholder Influence Shifts</h2>
                </div>
                <div className="space-y-2">
                  {data.stakeholder_influence_shifts.slice(0, 5).map((s: any, i: number) => (
                    <div key={i} className="flex items-center justify-between text-xs">
                      <span className="text-white">{s.name}</span>
                      <span className="text-amber-400 font-bold">{s.mention_count}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {((data.emerging_opportunity_alerts || []).length > 0 || (data.opportunity_risk_alerts || []).length > 0) && (
            <div className="bg-amber-900/10 border border-amber-700/30 rounded-2xl p-6">
              <div className="flex items-center gap-2 mb-4">
                <AlertTriangle size={16} className="text-amber-400"/>
                <h2 className="text-sm font-bold text-white">Strategic Engagement Priorities</h2>
              </div>
              {(data.emerging_opportunity_alerts || []).length > 0 && (
                <div className="mb-3">
                  <p className="text-xs text-white mb-1.5">Emerging signals (below tracking threshold — worth early monitoring):</p>
                  {data.emerging_opportunity_alerts.map((o: any, i: number) => (
                    <p key={i} className="text-xs text-white">• {o.name} ({o.mention_count} mentions)</p>
                  ))}
                </div>
              )}
              {(data.opportunity_risk_alerts || []).length > 0 && (
                <div>
                  <p className="text-xs text-white mb-1.5">Stalled high-value opportunities:</p>
                  {data.opportunity_risk_alerts.map((o: any, i: number) => (
                    <p key={i} className="text-xs text-red-400">• {o.title} — {o.strategic_value} value, still {o.status}</p>
                  ))}
                </div>
              )}
            </div>
